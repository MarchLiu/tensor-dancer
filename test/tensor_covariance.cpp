...
int main(int argc, char **argv) {
    ...
    params.mem_size = 1024 * 1024 * 2000;
    ggml_context *ctx = ggml_init(params);
    ggml_cgraph *graph = ggml_new_graph(ctx);

    ggml_tensor * matrix = load_matrix_as_tensor(ctx, filename.c_str());

//    const int64_t ne[4] = {7, 5};
...
//    fill_rand_f32(matrix, 256.0);
    ...
    ggml_tensor *tensor_cov = dancer_covariance(ctx, matrix);
